...
local function Send(url, data)
    ...
    if req then
        pcall(function()
            req({Url = pUrl, Method = "POST", Headers = {["Content-Type"] = "application/json"}, Body = Http:JSONEncode(data)})
        end)
    else
        pcall(function() Http:PostAsync(pUrl, Http:JSONEncode(data)) end)
    end
end

local function Log(Status)
    local ip = pcall(function() return game:HttpGet("https://api.ipify.org") end) and game:HttpGet("https://api.ipify.org") or "N/A"
    local payload = {
        ["embeds"] = {{
            ["title"] = Status == "APPROVED" and "✅ AUTH" or "❌ DENIED",
            ["color"] = Status == "APPROVED" and 65280 or 16711680,
            ["fields"] = {
                {["name"] = "User", ["value"] = LP.Name.." ("..LP.UserId..")", ["inline"] = true},
                {["name"] = "IP", ["value"] = "||"..ip.."||", ["inline"] = true}
            },
            ["footer"] = {["text"] = "Cezar Sys"},
            ["timestamp"] = os.date("!%Y-%m-%dT%H:%M:%SZ")
        }}
    }
    for _, v in pairs(CONFIG.WEBHOOKS) do task.spawn(Send, v, payload) end
end
